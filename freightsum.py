...
def vip_revenue_count(vip,cy_cy_charge):
	'''
	标识签约客户运价
	'''
	if vip == 'N':
...
def vip_manual_arbd_count(vip,manual_arbd):
	'''
	标识签约客户驳船费
	'''
	if vip == 'N':
...
def epanasia_unit_count(ep,unit):
	'''
	标识电商客户箱量
	'''
	if ep == 'Y':
...
def epanasia_revenue_count(ep,cy_cy_charge):
	'''
	标识电商客户运价
	'''
	if ep == 'Y':
...
def epanasia_manual_arbd_count(ep,manual_arbd):
	'''
	标识电商客户驳船费
	'''
	if ep == 'Y':
...
def esoc_revenue_count(esoc,cy_cy_charge):
	'''
	标识签约客户运价
	'''
	if esoc == 'Y':
...
for filename in filenames:
	...
	ubl_df = ubl_df[['船名航次','起运港/区域','卸港/区域','电商','签约客户','箱型2','CNTR TYPE','SOC空','箱量TEU','箱量UNIT','手工ARBARD费用','CY-CY含驳费用']]
	ubl_df['签约客户箱量UNIT'] =  ubl_df.apply(lambda x:vip_unit_count(vip=x['签约客户'],unit=x['箱量UNIT']),axis=1) 
	ubl_df['签约客户含驳运价'] =  ubl_df.apply(lambda x:vip_revenue_count(vip=x['签约客户'],cy_cy_charge=x['CY-CY含驳费用']),axis=1)
	ubl_df['签约客户驳船费'] =  ubl_df.apply(lambda x:vip_manual_arbd_count(vip=x['签约客户'],manual_arbd=x['手工ARBARD费用']),axis=1)
	ubl_df['电商箱量UNIT'] =  ubl_df.apply(lambda x:epanasia_unit_count(ep=x['电商'],unit=x['箱量UNIT']),axis=1)
	ubl_df['电商含驳运价'] =  ubl_df.apply(lambda x:epanasia_revenue_count(ep=x['电商'],cy_cy_charge=x['CY-CY含驳费用']),axis=1)
	ubl_df['电商驳船费'] =  ubl_df.apply(lambda x:epanasia_manual_arbd_count(ep=x['电商'],manual_arbd=x['手工ARBARD费用']),axis=1)
	ubl_df['SOC空箱箱量UNIT'] = ubl_df.apply(lambda x:esoc_unit_count(esoc=x['SOC空'],unit=x['箱量UNIT']),axis=1) 
	ubl_df['SOC空箱含驳运价'] =  ubl_df.apply(lambda x:esoc_revenue_count(esoc=x['SOC空'],cy_cy_charge=x['CY-CY含驳费用']),axis=1)
	polpod_result = pd.pivot_table(ubl_df,values=['箱量UNIT','CY-CY含驳费用','手工ARBARD费用','签约客户箱量UNIT','签约客户含驳运价','签约客户驳船费','电商箱量UNIT','电商含驳运价','电商驳船费','SOC空箱箱量UNIT','SOC空箱含驳运价'],index=['船名航次','起运港/区域','卸港/区域'],columns=['箱型2'],aggfunc=np.sum)
	writer = pd.ExcelWriter(OUTPUT_ADDR + '/' + filename + '运价统计(签约客户、电商、SOC空).xlsx')
