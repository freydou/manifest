...
OUTPUT_ADDR = r'E:\Python35\code\manifest'
...
def manual_arbd_filter(teu,arbd):
	'''
	去除不同payer导致的重复手工arbard费用
	'''
	if teu == 0:
		return 0
	else:
...
for filename in filenames:
	...
	bl_df = df[df['船名航次'] == df['主干线航次改']] #只取提单箱量
	bl_df['SOC箱量'] = bl_df.apply(lambda x:soc_counter(soc=x['SOC'],cn=x['箱型2']), axis=1) #计算soc箱量
	bl_df['SOC空箱量'] = bl_df.apply(lambda x:e_soc_counter(e_soc=x['SOC空'],cn=x['箱型2']), axis=1) #计算soc空箱量
	bl_df['手工ARBARD费用改'] = bl_df.apply(lambda x:manual_arbd_filter(teu=x['箱量TEU'],arbd=x['手工ARBARD费用']), axis=1) #去除不同payer导致的重复计算手工驳船费用
	bl_df['航次航向'] =  bl_df.apply(lambda x:direction_checker(svvd=x['船名航次']), axis=1) #判断航次航向
	
	basicdata = pd.pivot_table(bl_df,values=['箱量TEU','TTL AMT','IHL/IHD舱单原值','铁路费用舱单原值','手工ARBARD费用改','CY-CY含驳费用','SOC箱量','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','电商CHARGE','TSD舱单原值','ICD舱单原值','BUC舱单原值','SOC空箱量','DOC\DCI\SLF'],index=['船名航次'],aggfunc=np.sum)
	basicdata = pd.DataFrame(basicdata,columns=['箱量TEU','TTL AMT','IHL/IHD舱单原值','铁路费用舱单原值','手工ARBARD费用改','CY-CY含驳费用','SOC箱量','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','电商CHARGE','TSD舱单原值','ICD舱单原值','BUC舱单原值','SOC空箱量','DOC\DCI\SLF']) #对输出按指定排序
	writer = pd.ExcelWriter(OUTPUT_ADDR + '/' + '舱单测算透视结果.xlsx')
	basicdata.to_excel(writer,'舱单处理基础数据',merge_cells=False,index=True)
	
	epanasia_df = bl_df[bl_df['电商'] == 'Y'] #电商记录集
	s_epanasia_df = epanasia_df[epanasia_df['航次航向'] == '南行'] #南行电商记录集
	se_result = pd.pivot_table(s_epanasia_df,values=['箱量TEU','舱单运输类收入总和（含拖车和铁路）'],index=['船名航次'],aggfunc=np.sum)
	se_result = pd.DataFrame(se_result,columns=['箱量TEU','舱单运输类收入总和（含拖车和铁路）']) #对输出按指定排序
	se_result.to_excel(writer,'电商南行基础数据',merge_cells=False,index=True)
	n_epanasia_df = epanasia_df[epanasia_df['航次航向'] == '北行'] #北行电商记录集
	ne_result = pd.pivot_table(n_epanasia_df,values=['箱量TEU','舱单运输类收入总和（含拖车和铁路）'],index=['船名航次'],aggfunc=np.sum)
	ne_result = pd.DataFrame(ne_result,columns=['箱量TEU','舱单运输类收入总和（含拖车和铁路）']) #对输出按指定排序
	ne_result.to_excel(writer,'电商北行基础数据',merge_cells=False,index=True)
	
	non_epanasia_df = bl_df[bl_df['电商'] == 'N'] #线下记录集
	wuh_non_epanasia_df = non_epanasia_df[non_epanasia_df['起运港/区域'] == '长江中上游'] #POL为武汉区域的线下记录集
	wne_result =  pd.pivot_table(wuh_non_epanasia_df,values=['箱量TEU'],index=['船名航次'],aggfunc=np.sum)
	wne_result.to_excel(writer,'武汉区域线下基础数据',merge_cells=False,index=True)
	non_wuh_non_epanasia_df = non_epanasia_df[non_epanasia_df['起运港/区域'] != '长江中上游'] #POL为非武汉区域的线下记录集
	s_non_wuh_non_epanasia_df = non_wuh_non_epanasia_df[non_wuh_non_epanasia_df['航次航向'] == '南行'] #南行非武汉区域线下记录集
	snwe_result = pd.pivot_table(s_non_wuh_non_epanasia_df,values=['箱量TEU','舱单运输类收入总和（含拖车和铁路）'],index=['船名航次'],aggfunc=np.sum)
	snwe_result = pd.DataFrame(snwe_result,columns=['箱量TEU','舱单运输类收入总和（含拖车和铁路）']) #对输出按指定排序
	snwe_result.to_excel(writer,'非武汉区域线下南行基础数据',merge_cells=False,index=True)	
	n_non_wuh_non_epanasia_df = non_wuh_non_epanasia_df[non_wuh_non_epanasia_df['航次航向'] == '北行'] #北行非武汉区域线下记录集
	nnwe_result = pd.pivot_table(n_non_wuh_non_epanasia_df,values=['箱量TEU','舱单运输类收入总和（含拖车和铁路）'],index=['船名航次'],aggfunc=np.sum)
	nnwe_result = pd.DataFrame(nnwe_result,columns=['箱量TEU','舱单运输类收入总和（含拖车和铁路）']) #对输出按指定排序
	nnwe_result.to_excel(writer,'非武汉区域线下北行基础数据',merge_cells=False,index=True)
	'''
	included_y_df = bl_df[bl_df['有Y标识的佣金舱单总收入'] != 0] #included为Y的记录集
	included_n_df = bl_df[bl_df['有Y标识的佣金舱单总收入'] == 0] #included为N的记录集
	(bl_df[bl_df['有Y标识的佣金舱单总收入'] != 0]['OCB舱单原值'].sum() +  bl_df[bl_df['有Y标识的佣金舱单总收入'] != 0]['有Y标识的佣金舱单总收入'].sum())*0.7*0.00475 #计算include含Y标识且特定charge不为0的佣金
	included_n_cdyj = bl_df[bl_df['有Y标识的佣金舱单总收入'] == 0]['OCB舱单原值'].sum()*0.00475 #计算include含Y标识为0的佣金
