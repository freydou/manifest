FILES_DIR = 'E:/Python35/code/manifest/manifestexcel'
PARA_FILE = 'E:/Python35/code/manifest/parameter.xls'
OUTPUT_ADDR = 'E:/Python35/code/manifest/'
...
def matcher():
	...
	dictarbdtype = dictarbd['分类'] #由于to_dict函数默认有二层字典，因此通过第一层字典的键分类取第一层字典的值即整个第二层字典为实际分类匹配内容
	...
	return dictpol,dictpod,n_dictpol,n_dictpod,dictvip,dictport,dictarbd20,dictarbd40,dictarbdtype,dictbaseportchinesename,dictbaseportregion,dictbaseporthighregion,dict_sc_ts_nameclass,dict_cj_ts_nameclass,dictarbd_replace_route,dictdirection
...
def arbd_route_match_split(arbd,ct):
	'''
	用于分解筛选出来的arb和ard驳船路径的费用和路径
	'''
	if str(ct).startswith('2'):    #20尺非标箱暂用20尺标箱费率      
		arbd_dict = arbd_dict20
	elif str(ct).startswith('4'):    #40尺非标箱暂用40尺标箱费率
		arbd_dict = arbd_dict40
	'''
	if ct == '20尺标':          #使用20尺费率字典
		arbd_dict = arbd_dict20
	...
	if arbd == '': #若手工ARBARD路径为空则直接返回        
		...
		if len(routes) == 1:	 #若只有一层路径则直接匹配费率
			try:
				info = arbd_type[routes[0]] + '#' + routes[0] + '#' + str(arbd_dict[routes[0]])
				return info
			except KeyError:
				if routes[0] in arbd_replaced_dict.keys():  #若驳船路径在替换路径表中存在则进行替换并匹配费率
					info =  arbd_type[arbd_replaced_dict[routes[0]]] + '#' + arbd_replaced_dict[routes[0]] + '#' + str(arbd_dict[arbd_replaced_dict[routes[0]]])
				return info
		elif len(routes) > 1:
			i = 0
			while i<len(routes):
				if (i<len(routes)-2) and (routes[i].split('-')[1] == routes[i+1].split('-')[0]) and (routes[i+1].split('-')[1] == routes[i+2].split('-')[0]): #若在倒数第二层路径之前且连续两对前后层路径相连则匹配三层路径的总起运港和总卸港
					try:
						info = info + ',' + arbd_type[(routes[i].split('-')[0]+'-'+ routes[i+2].split('-')[1])] + '#' + (routes[i].split('-')[0]+'-'+ routes[i+2].split('-')[1]) + '#' + str(arbd_dict[(routes[i].split('-')[0]+'-'+ routes[i+2].split('-')[1])])
						i = i + 3 #若匹配到三层路径则下次循环跳过后两程路径
					except KeyError:  #若三层连续路径无费率则不改变重新进行两层连续路径的匹配
						try:
							info = info + ',' + arbd_type[(routes[i].split('-')[0]+'-'+ routes[i+1].split('-')[1])] + '#' + (routes[i].split('-')[0]+'-'+ routes[i+1].split('-')[1]) + '#' + str(arbd_dict[(routes[i].split('-')[0]+'-'+ routes[i+1].split('-')[1])])
							i = i + 2 #若匹配到两层路径则下次循环跳过后程路径
						except KeyError:
							try:  #若匹配不到两层连续路径的联程费率则匹配路径的各自费率-前半部分
								info = info + ',' + arbd_type[routes[i]] + '#' + routes[i] + '#' + str(arbd_dict[routes[i]])
								i = i + 1
							except KeyError:
								try:  #若匹配不到两层连续路径的联程费率则匹配路径的各自费率-后半部分
									info = info + ',' + arbd_type[routes[i+1]] + '#' + routes[i+1] + '#' + str(arbd_dict[routes[i+1]])
									i = i + 2
								except KeyError:
									i = i + 2
				elif (i<len(routes)-1) and (routes[i].split('-')[1] == routes[i+1].split('-')[0]): #若非最后一层路径且前后层路径相连则匹配两层路径的总起运港和总卸港
					...
					try:
						...
						if routes[i] in arbd_replaced_dict.keys():  #若驳船路径在替换路径表中存在则进行替换并匹配费率
							info = info + ',' + arbd_type[arbd_replaced_dict[routes[i]]] + '#' + arbd_replaced_dict[routes[i]] + '#' + str(arbd_dict[arbd_replaced_dict[routes[i]]])
						i = i + 1
			return info
...
def num_initial_charge_caculator(ttl_amount,cy_cy_plus_arbd,rail_arbd_amount,ihld_amount,tsd,psu,dps,bkf,ibs):
	'''
	倒算数字抬头收入
	'''
	return 	ttl_amount - cy_cy_plus_arbd - rail_arbd_amount - ihld_amount - tsd - psu - dps - bkf - ibs

def cargo_fee_caculator(teu,psu,dps,num_charge,epanasia_charge):
	'''
	计算货物费
	'''
	return 	teu * 339 + (psu + dps)/1.06 + (num_charge-epanasia_charge)/1.06 #19年1月费率从344改为339

def zero_buc_corrector(ct,buc,comm_related_charge):
	'''
	将BUC舱单原值为0的记录进行舱单运输类收入总和科目金额的更改
	'''
	if ct == 2:   #若BUC为0则根据值（1代表小箱，2代表大箱）进行调整
		if buc == 0:
...
def lhyj_caculator(teu,svvd,pol_region,epanasia,sum_charge):
	...
	if svvd[-1].upper() in ['S','E']:
		if epanasia == 'Y':
			lhyj = sum_charge * 0.018 * 0.6 #计算南行航次的电商货佣金
		else:
			if pol_region != '长江中上游':
				lhyj = sum_charge * 0.018 #计算南行航次下POL不为武汉区域的线下货佣金
			else:
				lhyj = 28.5 * teu #计算POL为武汉区域线下佣金（含南北行）
	elif svvd[-1].upper() in ['N','W']:
		if epanasia == 'Y':
			lhyj = sum_charge * 0.04 * 0.6 #计算北行航次的电商货佣金
		else:
			if pol_region != '长江中上游':
				lhyj = sum_charge * 0.04 #计算北行航次下POL不为武汉区域的线下货佣金
			else:
				lhyj = 28.5 * teu #计算POL为武汉区域线下佣金（含南北行）		
	return lhyj + 1.06 * teu #揽货佣金还需加上集运操作费 
...
def cdyj_caculator(ocb,comm_related_charge):
	...
	if comm_related_charge == 0:
		return ocb * 0.00475 #计算include含Y标识为0的佣金
	else:
		return (ocb + comm_related_charge) * 0.7 * 0.00475 #计算include含Y标识且特定charge不为0的佣金
		
time_start=time.time() #开始总运行时间的计时
	
filenames = os.listdir(FILES_DIR) #遍历文件夹获取所有文件的文件名

pol_dict,pod_dict,n_pol_dict,n_pod_dict,vip_dict,port_dict,arbd_dict20,arbd_dict40,arbd_type,baseport_dict,baseport_region_dict,baseport_highregion_dict,sc_ts_nameclass_dict,cj_ts_nameclass_dict,arbd_replaced_dict,direction_dict= matcher() #获取匹配字典
#遍历处理文件夹下所有文件
for filename in filenames:
	full_filename = FILES_DIR + '/' + filename
	df = pd.read_excel(full_filename)
	#以下对各列的空值进行格式化以避免异常情况
	df['船名航次']=df['船名航次'].fillna('')
	df['BL REF CDE']=df['BL REF CDE'].fillna('')
	df['CNTR NUM']=df['CNTR NUM'].fillna('')
	df['POR']=df['POR'].fillna('')
	df['POR CDE']=df['POR CDE'].fillna('')
	df['POL']=df['POL'].fillna('')
	df['POL CDE']=df['POL CDE'].fillna('')
	df['POD']=df['POD'].fillna('')
	df['POD CDE']=df['POD CDE'].fillna('')
	df['POD CDE改']=df['POD CDE改'].fillna('')
	df['FND']=df['FND'].fillna('')
	df['FND CDE']=df['FND CDE'].fillna('')
	df['起运港/区域']=df['起运港/区域'].fillna('')
	df['卸港/区域']=df['卸港/区域'].fillna('')
	df['电商']=df['电商'].fillna('')
	df['签约客户']=df['签约客户'].fillna('')
	df['箱型2']=df['箱型2'].fillna('')
	df['CNTR TYPE']=df['CNTR TYPE'].fillna('')
	df['ADPT']=df['ADPT'].fillna('')
	df['TS']=df['TS'].fillna('')
	df['SVVD1']=df['SVVD1'].fillna('')
	df['TS1']=df['TS1'].fillna('')
	df['TS1改']=df['TS1改'].fillna('')
	df['SVVD2']=df['SVVD2'].fillna('')
	df['TS2']=df['TS2'].fillna('')
	df['TS2改']=df['TS2改'].fillna('')
	df['SVVD3']=df['SVVD3'].fillna('')
	df['TS3']=df['TS3'].fillna('')
	df['TS3改']=df['TS3改'].fillna('')
	df['SVVD4']=df['SVVD4'].fillna('')
	df['SC NO']=df['SC NO'].fillna('')
	df['CSO NO']=df['CSO NO'].fillna('')
	df['SHIPPER']=df['SHIPPER'].fillna('')
	df['SHIPPER ID']=df['SHIPPER ID'].fillna('')
	df['PAYER']=df['PAYER'].fillna('')
	df['CONTROL PARTY']=df['CONTROL PARTY'].fillna('')
	df['TERMS']=df['TERMS'].fillna('')
	df['SOC']=df['SOC'].fillna('')
	df['SOC空']=df['SOC空'].fillna('')
	df['COMM']=df['COMM'].fillna('')
	df['BRIEF DESC']=df['BRIEF DESC'].fillna('')
	df['I/B Intermodal ']=df['I/B Intermodal '].fillna('')
	df['O/B Intermodal']=df['O/B Intermodal'].fillna('')
	df['空SVVD判断']=df['空SVVD判断'].fillna('')
	df['主干线航次']=df['主干线航次'].fillna('')
	df['手工ARBARD路径']=df['手工ARBARD路径'].fillna('')
	df['手工ARBARD费用']=df['手工ARBARD费用'].fillna(0)
	df['手工ARBARD信息']=df['手工ARBARD信息'].fillna('')
	#以下对箱量TEU和箱量UNIT列由于payer产生的零值进行处理，即直接按当前行的箱型2列重新进行赋值
	#df['箱量TEU'] = df.apply(lambda x:teu_formatter(teu=x['箱量TEU'],ct=x['箱型2']), axis=1)
	#df['箱量UNIT'] = [1 for x in df['CNTR NUM']]
	#以下只取提单箱量
	df['主干线航次改'] = df.apply(lambda x:svvd_formatter(msvvd=x['主干线航次']), axis=1)
	#以下将驳船路径和费用进行分解
	df['手工ARBARD费用分解'] = df.apply(lambda x:arbd_route_match_split(arbd=x['手工ARBARD路径'],ct=x['箱型2']),axis=1)
	
	bl_df = df[df['船名航次'] == df['主干线航次改']]
	#以下将不同payer的提单箱进行合并输出，还原成以一个箱子为一行
	result = pd.pivot_table(bl_df,values=['箱量TEU','箱量UNIT','CY-CY含驳费用','纯海加铁/驳','TTL AMT','OCB舱单原值','有Y标识的佣金舱单总收入','ARBARD舱单原值(不含铁路)','IHL/IHD舱单原值','铁路费用舱单原值','铁路ARB','铁路ARD','DCD舱单原值','ICD舱单原值','TSD舱单原值','HSD舱单原值','BUC舱单原值','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','电商CHARGE','DOC\DCI\SLF','舱单运输类收入总和（含拖车和铁路）','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识'],index=['船名航次','BL REF CDE','CNTR NUM','POR','POR CDE','POL','POL CDE','POD','POD CDE','POD CDE改','FND','FND CDE','起运港/区域','卸港/区域','流向','电商','签约客户','箱型2','CNTR TYPE','ADPT','TS','SVVD1','TS1','TS1改','SVVD2','TS2','TS2改','SVVD3','TS3','TS3改','SVVD4','SC NO','CSO NO','SHIPPER','SHIPPER ID','CONTROL PARTY','TERMS','SOC','SOC空','COMM','BRIEF DESC','I/B Intermodal ','O/B Intermodal','空SVVD判断','主干线航次','手工ARBARD路径','手工ARBARD费用','手工ARBARD信息','手工ARBARD费用分解'],aggfunc=np.sum)
	result = pd.DataFrame(result,columns=['箱量TEU','箱量UNIT','CY-CY含驳费用','纯海加铁/驳','TTL AMT','OCB舱单原值','有Y标识的佣金舱单总收入','ARBARD舱单原值(不含铁路)','IHL/IHD舱单原值','铁路费用舱单原值','铁路ARB','铁路ARD','DCD舱单原值','ICD舱单原值','TSD舱单原值','HSD舱单原值','BUC舱单原值','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','电商CHARGE','DOC\DCI\SLF','舱单运输类收入总和（含拖车和铁路）','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识']) #对输出按指定排序
	writer = pd.ExcelWriter(OUTPUT_ADDR + '/' + filename + '(提单箱量&合并).xlsx')
	result.to_excel(writer,'result',merge_cells=False,index=True)
	writer.save()
	print('第一步舱单转换已完成,用时'+str(int(time.time()-time_start))+'秒。以下进行第二步计算操作...')
	
	#以下进行第二步计算操作
	#读取转换后的舱单数据
	c_df = pd.read_excel(OUTPUT_ADDR + '/' + filename + '(提单箱量&合并).xlsx')
	...
	writer = pd.ExcelWriter(OUTPUT_ADDR + '/' + filename + '(计算结果).xlsx')
	c_df.to_excel(writer,'计算结果',merge_cells=False,index=False)
	writer.save()
