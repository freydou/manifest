...
for filename in filenames:
	...
	result = pd.pivot_table(ubl_df,values=['箱量UNIT','箱量TEU','CY-CY含驳费用','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识'],index=['船名航次','BL REF CDE','CNTR NUM','POR','POL','POD','FND','CNTR TYPE','TERMS'],aggfunc=np.sum)
	result = pd.DataFrame(result,columns=['箱量UNIT','箱量TEU','CY-CY含驳费用','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识']) #对输出按指定排序
	writer = pd.ExcelWriter(OUTPUT_ADDR + '/' + filename + 'IHLIHD一口价.xlsx')
