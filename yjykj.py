...
pol_dict,pod_dict,n_pol_dict,n_pod_dict,vip_dict,port_dict,arbd_dict20,arbd_dict40,baseport_dict,baseport_region_dict,baseport_highregion_dict,sc_ts_nameclass_dict,cj_ts_nameclass_dict,arbd_replaced_dict,direction_dict= matcher() #获取匹配字典
...
for filename in filenames:
	...
	#--以下输出结果--#
	result = pd.pivot_table(df,values=['箱量TEU','箱量UNIT','CY-CY含驳费用','TTL AMT','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','OCB舱单原值','BUC舱单原值','有Y标识的佣金舱单总收入','舱单运输类收入总和（含拖车和铁路）','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识'],index=['船名航次','BL REF CDE','CNTR NUM','POR','POL','POD','FND','起运港/区域','卸港/区域','电商','签约客户','箱型','箱型2','CNTR TYPE','TERMS','CSO NO','TS','SVVD1','TS1','SVVD2','TS2','SVVD3','TS3','SVVD4','SOC','CONTROL PARTY','SHIPPER','主干线航次','COMM','BRIEF DESC','起卸对','手工ARBARD路径','手工ARBARD费用','手工ARBARD信息'],aggfunc=np.sum)
	result = pd.DataFrame(result,columns=['CY-CY含驳费用','TTL AMT','箱量TEU','箱量UNIT','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','OCB舱单原值','BUC舱单原值','有Y标识的佣金舱单总收入','舱单运输类收入总和（含拖车和铁路）','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识']) #对输出按指定排序
	...
	#--以下输出佣金情况--#
	o_df = c_df[['船名航次','BL REF CDE','CNTR NUM','POL','POD','FND','起运港/区域','卸港/区域','电商','CNTR TYPE','TERMS','TS','SVVD1','TS1','SVVD2','TS2','SVVD3','TS3','SVVD4','主干线航次','箱量TEU','箱量UNIT','OCB舱单原值','BUC舱单原值','有Y标识的佣金舱单总收入','舱单运输类收入总和（含拖车和铁路）','舱单运输类收入总和（含拖车和铁路）改','揽货佣金','景华峰操作费','船代佣金','拖车货量']]
	writer = pd.ExcelWriter(OUTPUT_DIR + '/' + fn + '(佣金计算).xlsx')
	o_df.to_excel(writer,'佣金计算',merge_cells=False,index=False)
	writer.save()
	
	#--以下输出一口价情况--#
	ykj_df = c_df[['船名航次','BL REF CDE','CNTR NUM','POR','POL','POD','FND','CNTR TYPE','TERMS','TS','SVVD1','TS1','SVVD2','TS2','SVVD3','TS3','SVVD4','主干线航次','箱量TEU','箱量UNIT','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识']]
	writer = pd.ExcelWriter(OUTPUT_DIR + '/' + fn + '(一口价).xlsx')	
