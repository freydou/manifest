FILES_DIR = 'E:/Python33/code/manifest/resultexcel'
PARA_FILE = 'E:/Python33/code/manifest/parameter.xls'
OUTPUT_DIR = 'E:/Python33/code/manifest'
...
MAINLINE = ['IC0','IC1','IC2','IC4','IC5','IC6','IC7','IC8','IC9','IC10','IC11','IC12','IC15','IC16','IC17','IC18','IC19','IC20','IC21','IC22','IC23','IC25','IC26','IC27','IC28','CF3','IC36','IC38','IC39','IC40','CF66','CF67'] #干线航次集合，注意IC0代表虚拟空干线航次
...
for filename in filenames:
	...
	df['手工ARBARD费用分解'] = df.apply(lambda x:arbd_route_match_split(arbd=x['手工ARBARD路径'],ct=x['箱型2']),axis=1) #将驳船路径和费用进行分解
	
	#--以下输出结果--#
	result = pd.pivot_table(df,values=['箱量TEU','箱量UNIT','CY-CY含驳费用','TTL AMT','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值'],index=['船名航次','BL REF CDE','CNTR NUM','POL','POD','FND','起运港/区域','卸港/区域','电商','签约客户','箱型2','CNTR TYPE','CSO NO','TS','SVVD1','TS1','SVVD2','TS2','SVVD3','TS3','SVVD4','SOC','CONTROL PARTY','SHIPPER','主干线航次','COMM','BRIEF DESC','手工ARBARD路径','手工ARBARD费用','手工ARBARD信息','手工ARBARD费用分解'],aggfunc=np.sum)
	result = pd.DataFrame(result,columns=['CY-CY含驳费用','TTL AMT','箱量TEU','箱量UNIT','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值']) #对输出按指定排序
	writer = pd.ExcelWriter(OUTPUT_DIR + '/' + filename + '驳船分解处理后.xlsx')
	
	result.to_excel(writer,'驳船分解',merge_cells=False,index=True)
	
	writer.save()
	vessel_time_end=time.time() #结束总运行时间的计时
	print('处理'+filename+'用时共计'+str(int(vessel_time_end-vessel_time_start))+'秒。')
