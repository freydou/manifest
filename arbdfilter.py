...
PARA_FILE = 'E:/Python33/code/manifest/railallinbl.xlsx'
...
def railallin_matcher(bl):
	'''
	匹配提单是否铁路一口价
	'''
	if bl[4:] in dict_rail_all_in.keys():
		return dict_rail_all_in[bl[4:]]
	else:
		return ''
...
#以下读取一口价提单列表参数文件
df_rail_allin = pd.read_excel(PARA_FILE,sheetname='railallin',index_col=0) #注意需明示不使用默认索引，使用第一列为索引
dict_rail_all_in = df_rail_allin.to_dict(orient='dict')
dict_rail_all_in = dict_rail_all_in['类型']  #由于to_dict函数默认有二层字典，因此通过第一层字典的键取第一层字典的值即整个第二层字典为实际匹配内容
...
for filename in filenames:
	...
	ubl_df = bl_df[['船名航次','BL REF CDE','CNTR NUM','POR','POL','POD','FND','CNTR TYPE','TERMS','箱量TEU','箱量UNIT','CY-CY含驳费用','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识']]
	ubl_df['铁路一口价类型'] = ubl_df.apply(lambda x:railallin_matcher(bl=x['BL REF CDE']), axis=1)
	#ubl_df['铁路一口价成本'] = ubl_df.apply(lambda x:railallin_caculator(railallin=x['铁路一口价类型']), axis=1)
	result = pd.pivot_table(ubl_df,values=['箱量UNIT','箱量TEU','CY-CY含驳费用','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识'],index=['船名航次','BL REF CDE','CNTR NUM','POR','POL','POD','FND','CNTR TYPE','TERMS','铁路一口价类型'],aggfunc=np.sum)
	result = pd.DataFrame(result,columns=['箱量UNIT','箱量TEU','CY-CY含驳费用','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识']) #对输出按指定排序
	writer = pd.ExcelWriter(OUTPUT_ADDR + '/' + filename + '铁路一口价.xlsx')
