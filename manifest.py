FILES_DIR = 'E:/Python35/code/manifest/excel'
...
MAINLINE = ['IC0','IC1','IC2','IC4','IC5','IC6','IC7','IC8','IC9','IC10','IC11','IC12','IC15','IC16','IC17','IC18','IC19','IC20','IC21','IC22','IC23','IC25','IC26','IC27','IC28','CF3','IC36','IC38','IC39','IC40','IC41','CF66','CF1','IC24','CF6'] #干线航次集合，注意IC0代表虚拟空干线航次
BASEPORT_ROUTE_EXCEPTION = ['DCB-DOJ','DOJ-DCB','NSH-DOJ','DOJ-NSH','GLA-DOJ','NSH-HUM','HUM-NSH','GLA-HUM','DCB-HUM','HUI-NSH','NSH-HUI','HUI-DCB','DCB-HUI','HUI-GLA','GLA-HUI','NSH-GLA','NSH-DCB','DCB-NSH','FOC-FZN','FZN-FOC','TSN-CFD','DLC-HHA','APU-NGB','NGB-APU','TAO-LYG','WUC-NSH','NSH-WUC','DLC-DDG','DDG-DLC','XMN-FOC','SHA-TAG','TAG-SHA','DCB-WUC','WUC-DCB','TSN-HHA','TSN-SHP','WNZ-NGB'] #路径为基本港之间非干线（先后顺序重要）,涉及虎门东江仓惠州乌冲口南沙大铲湾高栏、天津曹妃甸、乍浦宁波、青岛连云港、大连丹东、厦门福州、上海太仓、大铲湾乌冲口、天津黄骅、天津秦皇岛、温州宁波
...
def ts_port_correcter(pre_vsl,tsport):
	...
	if tsport.strip().lower() == '': #若TS为空，则直接返回
		...
		ts = 'qinzhou'
	elif tsport.strip().lower() == 'huanghua': 
		if pre_vsl.split('-')[0] == 'IC26': #若TS中输的是huanghua且前置航线为IC26则由天津代替
			ts = 'xingang'
		else:
			ts = 'huanghua'
	elif tsport.strip().lower() in ['dandong','dongying','weihai','longkou']: #若TS中输的是渤海湾驳点(除潍坊)且前置航线为IC15则由大连代替
		if pre_vsl.split('-')[0] in ['IC15']:
			ts = 'dalian'
		else:
			ts = tsport.strip().lower()
	elif tsport.strip().lower() == 'shantou': 
		if pre_vsl.split('-')[0] in ['IC7']: #若TS中输的是shantou且前置航线为IC7则由厦门代替
			ts = 'xiamen'
			...
			ts = 'shantou'
	elif tsport.strip().lower() == 'quanzhou': #若TS中输的是quanzhou且前置航线为IC7则由厦门代替
		if pre_vsl.split('-')[0] in ['IC7']:
			ts = 'xiamen'
		else:
			ts = 'quanzhou'
		...
		if pre_vsl.split('-')[0] == 'IC11':
			ts = 'qingdao'
		elif pre_vsl.split('-')[0] in ['IC15']: #若前置航线为IC15则由大连代替
			ts = 'dalian'
		else:
			ts = 'weifang'
		...
		if pre_vsl.split('-')[0] == 'IC12':
			...
			ts = 'laizhou'
	elif tsport.strip().lower() == 'zhapu': #若TS中输的是zhapu且前置航线为IC9或IC15则由宁波代替
		if pre_vsl.split('-')[0] in ['IC15','IC9']:
			ts = 'ningbo'
		...
		if pre_vsl.split('-')[0] == 'IC16':
			ts = 'gaolan'
		elif pre_vsl.split('-')[0] == 'IC7':  #若TS中输的是dongjiang且前置航线为IC7则由厦门代替
			ts = 'xiamen'
		else:
			ts = 'nansha'
		...
		if pre_vsl.split('-')[0] in ['IC16','IC17','IC20']: #若前置航线为IC16、IC17、IC20则由太仓代替
			ts = 'taicang'
		elif pre_vsl.split('-')[0] in ['IC8','IC6','IC7','IC5','CF66']:  #若前置航线为IC6或IC8或IC7或IC5或CF66则由上海代替
			ts = 'shanghai'
		else:
			ts = tsport.strip().lower()
			...
			if vessel_list[i].strip() in ['- -','--']:               #若当前航次为虚拟空航次
				if i == 0:                                #代表当前处理的是第一段路径即起运港到第一中转港
					if (port_list[0] in baseport_dict.keys()) and (port_list[3] in baseport_dict.keys()) and (port_list[0]!=port_list[3]):   #若当前路径段起运港和目的港均为基本港，则该虚拟空航次为干线
						if (port_list[0] + '-' + port_list[3]) not in arbd_dict20.keys():  #排除基本港支线的特殊情况
					...
					if  port_list[-1] == port_list[1]:     #若最后一中转港等于卸港，则最后一段路径取最后一中转港到最终目的港
						if (port_list[-1] in baseport_dict.keys()) and (port_list[2] in baseport_dict.keys()) and (port_list[-1]!=port_list[2]):   #若当前路径段起运港和目的港均为基本港，则该虚拟空航次为干线
							if (port_list[-1] + '-' + port_list[2]) not in arbd_dict20.keys(): #排除基本港支线的特殊情况
						...
						if (port_list[-1] in baseport_dict.keys()) and (port_list[1] in baseport_dict.keys()) and (port_list[-1]!=port_list[1]):   #若当前路径段起运港和目的港均为基本港，则该虚拟空航次为干线
							if (port_list[-1] + '-' + port_list[1]) not in arbd_dict20.keys(): #排除基本港支线的特殊情况
					...
					if (port_list[i+2] in baseport_dict.keys()) and (port_list[i+3] in baseport_dict.keys()) and (port_list[i+2]!=port_list[i+3]):  #若当前路径段起运港和目的港均为基本港，则该虚拟空航次为干线
						if (port_list[i+2] + '-' + port_list[i+3]) not in arbd_dict20.keys(): #排除基本港支线的特殊情况	
	...
	if int(ts) == 0:     #若无中转，则svvd1必定为主干线航次
		...
		for i in range(0,vessel_no):
			if vessel_list[i].strip() in ['- -','--']:               #若当前航次为虚拟空航次
				if i == 0:                                #代表当前处理的是第一段路径即起运港到第一中转港
					if (port_list[0] in baseport_dict.keys()) and (port_list[3] in baseport_dict.keys()) and (port_list[0]!=port_list[3]):   #若当前路径段起运港和目的港均为基本港，则该虚拟空航次为干线
						if (port_list[0] + '-' + port_list[3]) not in arbd_dict20.keys(): #排除虎门/东江仓到南沙/高栏/大铲湾这种基本港支线的特殊情况
							vessel_list[i] = 'IC0-'        #用IC0标示虚拟空干线航次
				elif i == vessel_no - 1:                  #代表当前处理的是最后一段路径即最后一中转港到卸港/最终目的港
					if  port_list[-1] == port_list[1]:     #若最后一中转港等于卸港，则最后一段路径取最后一中转港到最终目的港
						if (port_list[-1] in baseport_dict.keys()) and (port_list[2] in baseport_dict.keys()) and (port_list[-1]!=port_list[2]):   #若当前路径段起运港和目的港均为基本港，则该虚拟空航次为干线
							if (port_list[-1] + '-' + port_list[2]) not in arbd_dict20.keys(): #排除虎门/东江仓到南沙/高栏/大铲湾这种基本港支线的特殊情况
								vessel_list[i] = 'IC0-'       #用IC0标示虚拟空干线航次
					else:                                  #若最后一中转港不等于卸港，则最后一段路径取最后一中转港卸港到卸港
						if (port_list[-1] in baseport_dict.keys()) and (port_list[1] in baseport_dict.keys()) and (port_list[-1]!=port_list[1]):   #若当前路径段起运港和目的港均为基本港，则该虚拟空航次为干线
							if (port_list[-1] + '-' + port_list[1]) not in arbd_dict20.keys(): #排除虎门/东江仓到南沙/高栏/大铲湾这种基本港支线的特殊情况
								vessel_list[i] = 'IC0-'       #用IC0标示虚拟空干线航次
				else:                                     #代表当前处理的是中间路径即中转港到中转港
					if (port_list[i+2] in baseport_dict.keys()) and (port_list[i+3] in baseport_dict.keys()) and (port_list[i+2]!=port_list[i+3]):  #若当前路径段起运港和目的港均为基本港，则该虚拟空航次为干线
						if (port_list[i+2] + '-' + port_list[i+3]) not in arbd_dict20.keys(): #排除虎门/东江仓到南沙/高栏/大铲湾这种基本港支线的特殊情况
							vessel_list[i] = 'IC0-'          #用IC0标示虚拟空干线航次
		...
		if len(main_vsl_ports_dict) == 0:  #若干线航次字典为空则返回异常说明（理论上不应该出现）
			...
			max_distance = 0
			try:
				seq = 0 #用于记录最大距离的主干线航次的标号
				for i in range(0,len(ml_vessels)):
					if portdistance_dict[ml_ports[i]] > max_distance:
						max_distance = portdistance_dict[ml_ports[i]]
						seq = i
					else:
						continue
				return ml_vessels[seq]
			except KeyError: #处理干线路径未匹配到海里数的情况
				return ml_ports[i] + '未维护海里数，无法判断主干线；'
			'''
...
def arbd_route_filter(cn,ct,ts,pol,v1,ts1,v2,ts2,v3,ts3,v4,pod,fnd,ml,por):
	...
	'''
		...
		#return ''
	if ml.startswith('CF') and ml.split('-')[0] not in ['CF3','CF66','CF67','CF1','CF6']: #若主干线为CF航向也无驳船（对于内贸来说）,注意CF3、CF66、CF67目前为内贸运营航次视为IC干线
		return ''
	else:
		input_port_list = [pol,pod,fnd,ts1,ts2,ts3] #所有舱单输入的涉及港口包含空 
		input_vessel_list = [v1,v2,v3,v4] #所有舱单输入的承载航次包含空
...
for filename in filenames:
	...
	df['IHL/IHD舱单原值'] = df.apply(lambda x:ihld_filter(charge_type=x['CHRG TYPE'],charge_amount=x['TTL AMT']), axis=1)
	...
	#--以下输出结果--#
	result = pd.pivot_table(df,values=['箱量TEU','箱量UNIT','CY-CY含驳费用','TTL AMT','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','IBS返还费率'],index=['船名航次','BL REF CDE','CNTR NUM','POL','POD','FND','起运港/区域','卸港/区域','电商','签约客户','箱型','箱型2','CNTR TYPE','CSO NO','TS','SVVD1','TS1','SVVD2','TS2','SVVD3','TS3','SVVD4','SOC','CONTROL PARTY','SHIPPER','主干线航次','COMM','BRIEF DESC','起卸对','手工ARBARD路径','手工ARBARD费用','手工ARBARD信息'],aggfunc=np.sum)
	result = pd.DataFrame(result,columns=['CY-CY含驳费用','TTL AMT','箱量TEU','箱量UNIT','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','IBS返还费率','IBS返还值']) #对输出按指定排序
	...
	arbd_result = pd.pivot_table(df,values=['箱量TEU','箱量UNIT','CY-CY含驳费用','纯海加铁/驳','TTL AMT','OCB舱单原值','有Y标识的佣金舱单总收入','ARBARD舱单原值(不含铁路)','IHL/IHD舱单原值','铁路费用舱单原值','铁路ARB','铁路ARD','DCD舱单原值','ICD舱单原值','TSD舱单原值','HSD舱单原值','BUC舱单原值','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','IBS返还费率','电商CHARGE','DOC\DCI\SLF','舱单运输类收入总和（含拖车和铁路）','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识'],index=['船名航次','BL REF CDE','CNTR NUM','POR','POR CDE','POL','POL CDE','POD','POD CDE','POD CDE改','FND','FND CDE','起运港/区域','卸港/区域','流向','电商','签约客户','箱型2','CNTR TYPE','ADPT','TS','SVVD1','TS1','TS1改','SVVD2','TS2','TS2改','SVVD3','TS3','TS3改','SVVD4','SC NO','CSO NO','SHIPPER','SHIPPER ID','CONTROL PARTY','TERMS','SOC','COMM','BRIEF DESC','I/B Intermodal ','O/B Intermodal','空SVVD判断','主干线航次','手工ARBARD路径','手工ARBARD费用','手工ARBARD信息'],aggfunc=np.sum)
	arbd_result = pd.DataFrame(arbd_result,columns=['箱量TEU','箱量UNIT','CY-CY含驳费用','纯海加铁/驳','TTL AMT','OCB舱单原值','有Y标识的佣金舱单总收入','ARBARD舱单原值(不含铁路)','IHL/IHD舱单原值','铁路费用舱单原值','铁路ARB','铁路ARD','DCD舱单原值','ICD舱单原值','TSD舱单原值','HSD舱单原值','BUC舱单原值','PSU舱单原值','DPS舱单原值','BKF舱单原值','IBS舱单原值','IBS返还费率','电商CHARGE','DOC\DCI\SLF','舱单运输类收入总和（含拖车和铁路）','IHL金额标识','ARB金额标识','IHD金额标识','ARD金额标识']) #对输出排序
	writer = pd.ExcelWriter(OUTPUT_DIR + '/' + fn + '（处理后）.xlsx')
	
	#df.to_excel(writer,'result',merge_cells=False,index=False)
	result.to_excel(writer,'运价跟踪输出模板',merge_cells=False,index=True)
	...
	'''
	trunkset = set(df_result['主干线航次'])
	oddtrunk = 'False' 
	#判断是否存在需人工判断的主航次 
	for trunk in trunkset:	
		if trunk.startswith('IC') or trunk.startswith('CF'):
			continue
		else:
			oddtrunk = 'True'		
	#获取当前航次并转变航次格式
	if len(df_result['船名航次'][0]) == 10:
		trunkname = df_result['船名航次'][0][0:3] + '-' + df_result['船名航次'][0][3:6] + '-' + df_result['船名航次'][0][6:]
	elif len(df_result['船名航次'][0]) == 11:
		trunkname = df_result['船名航次'][0][0:4] + '-' + df_result['船名航次'][0][4:7] + '-' + df_result['船名航次'][0][7:]
	#获取航次提单箱量单收信息
	own_df_result = df_result[df_result['主干线航次'] == trunkname] #只取主干线航次为当前航次的记录 
	own_teu = own_df_result['箱量TEU'].sum() #提单箱量
	if own_teu == 0:
		own_full_rpt = 0
		own_cy_rpt = 0
	else:
		own_full_rpt = own_df_result['TTL AMT'].sum()/own_teu #提单箱量全口径单收
		own_cy_rpt = own_df_result['CY-CY含驳费用'].sum()/own_teu #提单箱量CY-CY含驳单收

	vip_df = own_df_result[own_df_result['签约客户']!='N'] #长约客户记录
	not_vip_df = own_df_result[own_df_result['签约客户']=='N'] #非长约客户记录
	epanasia_df = own_df_result[own_df_result['电商']=='Y'] #电商记录
	not_epanasia_df = own_df_result[own_df_result['电商']=='N'] #非电商记录
	if vip_df.empty:
		vip_teu = 0
		vip_cy_rpt = 0
	else:
		vip_teu = vip_df['箱量TEU'].sum() #长约客户箱量
		vip_cy_rpt = vip_df['CY-CY含驳费用'].sum()/vip_teu #长约客户CY-CY含驳单收
	if not_vip_df.empty:
		not_vip_teu = 0
		not_vip_cy_rpt = 0
	else:
		not_vip_teu = not_vip_df['箱量TEU'].sum() #非长约客户箱量
		not_vip_cy_rpt = not_vip_df['CY-CY含驳费用'].sum()/not_vip_teu #非长约客户CY-CY含驳单收
	if epanasia_df.empty:
		epanasia_teu = 0
		epanasia_cy_rpt = 0
	else:
		epanasia_teu = epanasia_df['箱量TEU'].sum() #电商客户箱量
		epanasia_cy_rpt = epanasia_df['CY-CY含驳费用'].sum()/epanasia_teu #电商客户CY-CY含驳单收
	if not_epanasia_df.empty:
		not_epanasia_teu = 0
		not_epanasia_cy_rpt = 0
	else:
		not_epanasia_teu = not_epanasia_df['箱量TEU'].sum() #非电商客户箱量
		not_epanasia_cy_rpt = not_epanasia_df['CY-CY含驳费用'].sum()/not_epanasia_teu #非电商客户CY-CY含驳单收
		
	info_df = pd.DataFrame({'船名航次':[df_result['船名航次'][0]],'提单箱量':[own_teu],'CY-CY含驳单收':[own_cy_rpt],'全口径单收':[own_full_rpt],'是否存在人工判断主干线航次':[oddtrunk],'长约客户箱量':[vip_teu],'长约客户CY-CY含驳单收':[vip_cy_rpt],'非长约客户箱量':[not_vip_teu],'非长约客户CY-CY含驳单收':[not_vip_cy_rpt],'电商客户箱量':[epanasia_teu],'电商客户CY-CY含驳单收':[epanasia_cy_rpt],'非电商客户箱量':[not_epanasia_teu],'非电商客户CY-CY含驳单收':[not_epanasia_cy_rpt]}) #生成航次基本信息df
	info_df = info_df[['船名航次','提单箱量','CY-CY含驳单收','全口径单收','是否存在人工判断主干线航次','长约客户箱量','长约客户CY-CY含驳单收','非长约客户箱量','非长约客户CY-CY含驳单收','电商客户箱量','电商客户CY-CY含驳单收','非电商客户箱量','非电商客户CY-CY含驳单收']] #重排列顺序
	info_df.to_excel(writer,'航次基本信息',merge_cells=False,index=False)
	'''
	writer.save()
