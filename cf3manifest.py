FILES_DIR = 'E:/Python33/code/manifest/excel'
...
branch = {'AAT':'拉非贸易区','AAW':'拉非贸易区','AAS':'拉非贸易区','AEA':'拉非贸易区','AWA':'拉非贸易区','ACA':'拉非贸易区','ILA':'拉非贸易区','TAA':'拉非贸易区','TAW':'拉非贸易区','AUS':'亚太贸易区','AUZ':'亚太贸易区','IAG':'亚太贸易区','IAR':'亚太贸易区','IAA':'新鑫海公司','IAN':'新鑫海公司','IAP':'新鑫海公司','IAK':'新鑫海公司','IAH':'新鑫海公司','AEM':'欧洲贸易区','AEU':'欧洲贸易区','IET':'欧洲公司','TAM':'欧洲公司','TAN':'欧洲公司','TPE':'美洲贸易区','TPW':'美洲贸易区','ICT':'泛亚公司','CJC':'泛亚公司支线','CNJ':'泛亚公司中日','CCN':'泛亚公司港销','IAW':'泛亚公司台湾'}
...
for filename in filenames:
	...
	df['TRADE LANE'] = df['TRADE LANE'].fillna('')#对空值赋值
	...
	df['贸易区/公司'] = [branch[x] for x in df['TRADE LANE']]
	...
	#--以下输出结果--#
	result = pd.pivot_table(df,values=['箱量TEU','箱量UNIT'],index=['船名航次','BL REF CDE','CNTR NUM','POL','POD','FND','箱型','箱型2','CNTR TYPE','TS','SVVD1','TS1','SVVD2','TS2','SVVD3','TS3','SVVD4','贸易区/公司','TRADE LANE'],aggfunc=np.sum)
	writer = pd.ExcelWriter(OUTPUT_DIR + '/' + fn + '（CF3处理后）.xlsx')
	result.to_excel(writer,'输出',merge_cells=False,index=True)
	writer.save()
	vessel_time_end=time.time() #结束总运行时间的计时
	print('处理'+df['船名航次'][0]+'用时共计'+str(int(vessel_time_end-vessel_time_start))+'秒。')
